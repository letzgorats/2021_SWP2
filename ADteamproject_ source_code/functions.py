...
def scrape_weather():
    ...
    # 현재 oo도(최저 기온,최고 기온)
    curr_temp = soup.find(
        "div", attrs={"class": "temperature_text"}).get_text()[4:]
    min_temp = soup.find("span", attrs={"class": "lowest"}).get_text()  # 최저
    max_temp = soup.find("span", attrs={"class": "highest"}).get_text()  # 최고

    rainfall = soup.find_all("span", attrs={"class": "weather_left"})
    morning_rain_rate = rainfall[0].get_text()[4:]
    afternoon_rain_rate = rainfall[1].get_text()[4:]
    # morning_rain_rate = soup.find(
    #     "span", attrs={"class": "rainfall"}).get_text().strip()  # 오전 강수량

    # afternoon_rain_rate = soup.find(
    #     "span", attrs={"class": "rainfall"}).get_text().strip()  # 오후 강수량

    # 미세먼지
    dust = soup.find_all(
        "li", attrs={"class": "item_today level2"})
    pm10 = dust[0].find("span").get_text()  # 미세먼지
    pm25 = dust[1].find("span").get_text()  # 초미세먼지
